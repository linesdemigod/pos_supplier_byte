...
@section('content')
    <x-breadcrumb title="Request Item" subtitle='Add Request Item' name='Request Item' href='storerequest.index' />
    <section class="py-4">
        <div class="container">
            <div class="row">
                    ...
                    <div class="card border-1 mb-3 border">
                        <div class="card-body">
                                            ...
                                            @unless (count($warehouses) == 0)
                                                @foreach ($warehouses as $warehouse)
                                                    <option value="{{ $warehouse->id }}"
                                                        {{ $warehouse->id === $item->warehouse_id ? 'selected' : '' }}>
                            ...
                            <div class="table-responsive" data-simplebar style="max-height: 400px">
                                <table class="table-centered table">
                                    ...
                                    <tbody id="cart-table">
                                        ...
                                        @endphp
                                        @foreach ($storeItems->storeRequestDetails as $data)
                                            @php
                                                $subtotal = $data->item->price * $data->requested_quantity;
                                                $total += $subtotal;
                                            @endphp
                                            <tr role="button">
                                                ...
                                                <td>
                                                    <input type="text" class="form-control quantity-cart"
                                                        value="{{ $data->requested_quantity }}" min="1"
                                                        maxlength="3" max="100"
                                                        onkeypress="return event.charCode >= 48 && event.charCode <= 57" />
                                                </td>
                                                <td>{{ $subtotal }}</td>
                                                <td><span class="text-danger fw-bold fs-4"><i
                                                            class="fas fa-trash delete-btn"
                                                            data-id="${holdItem.item_id}"></i></span></td>
                                ...
                                <div class="row g-3 justify-content-end mb-2">
                                    ...
                                    <div class="col-auto">
                                        <input type="text" id="total-grandtotal" name="net"
                                            class="form-control net-total" readonly="" value=" {{ $total }}">
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <button class="btn btn-primary" id="update-place-order">Update Request</button>
                            </div>
                        </div>
                    </div>
                </div>
