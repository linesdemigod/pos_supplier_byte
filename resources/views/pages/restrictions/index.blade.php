...
@section('content')
    <x-breadcrumb title="Time Restriction" subtitle='Time Restrictions' name='Restriction' href='restriction.index' />
    <section class="py-4">
        <div class="container">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    <div class="card border-1 border">
                        <div class="card-body">
                            <h3 class="py-2 text-center">Time Restriction</h3>
                            <form action="{{ route('restriction.update', $restriction->id) }}" method="Post">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="form-group mb-3">
                                    <label for="user_exemptions" class="form-label">User Allowed (JSON Array)</label>
                                    <textarea readonly name="user_exemptions" id="user_exemptions" class="form-control" rows="5" required>{{ old('user_exemptions', json_encode($restriction->user_exemptions)) }}</textarea>
                                    <small class="form-text text-muted">Enter user IDs or roles in JSON format, e.g., [1, 2,
                                        3]</small>
                                    @error('user_exemptions')
...
@section('script')
    <script>
        document.getElementById('add_user').addEventListener('click', function() {
            ...
            const textarea = document.getElementById('user_exemptions');
        ...
        document.getElementById('user_exemptions').addEventListener('input', function() {
            const textarea = this;
            try {
