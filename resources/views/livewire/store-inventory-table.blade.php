                    ...
                    <tbody>
                        ...
                        @foreach ($inventories as $inventory)
                            <tr wire:key="{{ $inventory->id }}">
                                ...
                                <td> {{ $inventory->item->price ?? 0 }} </td>
                                <td>
                                    <span
                                        class="{{ $inventory->quantity <= 0 ? 'text-danger' : ($inventory->quantity <= $inventory->low_stock_threshold ? 'text-primary' : '') }}">
                                        ...
                                        @else
                                            In Stock
                                        @endif
                                        {{-- {{ $inventory->quantity <= 0 ? 'Out of Stock' : ($inventory->quantity <= $inventory->low_stock_threshold ? 'Running out of stock' : '') }} --}}
                                    </span>
                                </td>
                                ...
                                <td>
                                    <div class="d-flex">
                                        @can('storeInventory.update')
                                            <a href="{{ route('storeinventory.edit', $inventory->id) }}"
